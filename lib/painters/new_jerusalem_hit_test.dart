...
import 'new_jerusalem_painter.dart' show NewJerusalemPainter, cityVoxels;

/// Hit-test utilities for the New Jerusalem isometric view.
/// Mirrors the interface of [BlockHitTest] but operates on the city voxel list.
class NewJerusalemHitTest {
  static const double blockSize = NewJerusalemPainter.blockSize;
  static const double _cos30 = 0.866;
  static const double _sin30 = 0.5;

  static int get _totalStructural => NewJerusalemPainter.totalStructuralBlocks;

  // ---------------------------------------------------------------------------
  // Projection — must match NewJerusalemPainter exactly.
  // ---------------------------------------------------------------------------
  ...
  static Offset origin(Size canvasSize) {
    return Offset(canvasSize.width / 2, canvasSize.height * 0.62);
  }
  ...
  static BlockCoord? hitTest(Offset scenePoint, Size canvasSize,
    ...
    final voxels = List.of(cityVoxels)
      ..sort((a, b) {
        final depthA = _depthKey(
            a.x.toDouble(), a.y.toDouble(), a.z.toDouble(), angle);
        final depthB = _depthKey(
            b.x.toDouble(), b.y.toDouble(), b.z.toDouble(), angle);
  ...
  static int toBlockIndex(BlockCoord coord) {
    final voxel = cityVoxels.firstWhere(
      (v) => v.x == coord.x && v.y == coord.y && v.z == coord.z,
      orElse: () => throw ArgumentError(
          'No city voxel at (${coord.x}, ${coord.y}, ${coord.z})'),
    );
